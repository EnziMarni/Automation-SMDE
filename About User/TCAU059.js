...
/**
 * This script tests the login, navigation to 'About Me', and back to the home section of the web application
 * according to the ISO/IEC/IEEE 29119 standards.
 *
 * Test Case ID: TCAU059
 * Test Objective: Verify that a user can log in, navigate to the 'About Me' section, and return to the home page successfully.
 * Pre-conditions: The user must have valid login credentials.
 * Post-conditions: The user should successfully navigate to the 'About Me' section and back to the home page.
 *
 * Steps:
 ...
 * 8. Click the 'Back to Home' button.
 * 9. Verify navigation back to the home page.
 */
...
(async function aboutUser() {
  ...
  try {
    ...
    // Step 8: Click the 'Back to Home' button
    await driver.wait(until.elementLocated(By.css("a.btn.btn-primary[href*='home']")), 10000).click();
    console.log("Navigasi kembali ke halaman utama berhasil!");

    // Step 9: Verify navigation back to the home page
    await driver.wait(until.urlContains("home"), 10000);
    pageTitle = await driver.getTitle();
    if (pageTitle.includes("Home")) {
      console.log("Navigasi ke halaman utama berhasil!");
    } else {
      throw new Error("Gagal navigasi ke halaman utama");
    }
  } catch (error) {
